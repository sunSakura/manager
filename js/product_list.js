    ...
    success: function(pager){
      ...
      $.each(pager.data, function(i, l){
        ...
        html += `
            ...
            <td><p class="fname" title="${l.pname}">${l.pname}</p></td>
            <td><p class="title" title="${l.title}">${l.title}</p></td>
            <td><p class="spec" title="${l.color}">${l.color}</p></td>
            <td><p class="spec" title="${l.size}">${l.size}</p></td>
...
$("#table-laptop").on("click","[data-action='delete']",function(e){
   ...
   var pids = tr.find("[class='pids']");
   var rs = window.confirm("您确认要删除商品编号为："+pids.html()+"的对应记录吗?");
   if(!rs){
    return;
   }
   $.ajax({
...
$("#table-laptop").on("click","[data-action='detail']",function(e){
  ...
  $.ajax({
    ...
    success:function(data){
      ...
      div.find(".title").html(data.sub_title);
      div.find(".pprice").html(data.price);
      div.find(".pcategory").html(data.color);
      div.find(".pos").html(data.size);
      div.show(500);
      console.log(div);
    },
    error:function(){
